...
export function LandscapingCheckoutModal({ open, onClose }: Props) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/landscaping-lead", {
        method: "POST",
  ...
  if (success) {
    const waMessage =
      "Hola 🌿 Vengo de la web de Jardín Amazónico.\n\nMe interesa su servicio de paisajismo y diseño de interiores.";
      ...
      <Modal open={open} onClose={onClose} maxWidth="max-w-lg" title="Datos recibidos">
        <div className="px-6 md:px-10 py-10 text-center">
          ...
          <p className="mt-3 text-ja-ink/80">
            Continúa por WhatsApp para conversar sobre tu proyecto de paisajismo
  ...
  return (
    <Modal open={open} onClose={onClose} maxWidth="max-w-lg" title="Cuéntanos sobre tu proyecto">
      <div className="px-6 md:px-10 py-8 md:py-10">
        ...
        <form className="mt-6 space-y-4" onSubmit={submit}>
          ...
          <Field label="Sobre tu espacio (opcional)">
            <textarea
              rows={3}
              maxLength={500}
              value={message}
              onChange={(e) => setMessage(e.target.value)}
              placeholder="Tipo de espacio, metros cuadrados, idea general…"
